...
/**
 ...
 user는 LAZY 연관관계라 접근 방식에 따라 동작이 달라진다
 두 가지 팩토리 메서드를 함께 제공(학습용)

 from(): getUser().getId()만 사용. FK가 chat_logs에 이미 있어 프록시를 초기화하지 않으므로 fetch join 없이도 안전
...
public record ChatLogResponse(
    ...
    /**
     * getId 버전 — username은 채움 ㄴㄴ
     * LAZY 프록시의 식별자만 읽으므로 fetch join이 필요 없음
     */
    ...
    /**
     * getName(getUsername) 버전 — fetch join으로 user가 미리 로딩되어 있어야 함
     */
